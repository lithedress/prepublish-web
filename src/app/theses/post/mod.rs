...
fn get_check_abstraction(abstraction: UseStateHandle<AttrValue>, is_good_abstraction: UseStateHandle<bool>) -> impl Fn(InputEvent) {
    move |e: InputEvent| {
...
fn get_check_title(title: UseStateHandle<AttrValue>, is_good_title: UseStateHandle<bool>) -> impl Fn(InputEvent) {
    move |e: InputEvent| {
        if let Some(input) = e.target_dyn_into::<web_sys::HtmlInputElement>() {
